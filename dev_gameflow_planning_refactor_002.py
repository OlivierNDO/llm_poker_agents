...
class Hand:
    ...
    def _award_with_side_pots(self, active_players: List['Player']) -> str:
        """Handle side pot distribution when players are all-in for different amounts"""
        ...
        # Sort by contribution amount to process side pots
        contribution_levels = sorted(set(ph['contribution'] for ph in player_hands))
        ...
        for level in contribution_levels:
            ...
            # Calculate this side pot size
            eligible_players = [ph for ph in player_hands if ph['contribution'] >= level]
            pot_increment = level - last_level
            side_pot_size = pot_increment * len(eligible_players)
            
            # Find best hand among eligible players
            eligible_sorted = sorted(eligible_players, 
            ...
            pot_winners = [ph for ph in eligible_sorted 
        ...
        # Safety check - should not have remaining chips
        if remaining_pot > 0:
            self.logger.warning(f"Warning: {remaining_pot} chips unaccounted for in pot distribution")
...
Output JSON only:"""
    ...
    def DEPRECATED_maybe_reconsider_bluff(
        self,
        combined_info: str,
        decision: dict,
        allowed_actions: list,
        ...
        prompt = f"""{self.player_profile}
    
    You chose a passive action. Reconsider once whether switching to a bluff or semi-bluff is better.
    ...
    Start your response with {{"""
        if self.log_prompts:
            self.logger.info('{self.name}: Bluff reconsideration prompt submitted:')
            self.logger.info(prompt)
        ...
        if not isinstance(action, str):
            return None
        try:
            atype = ActionType(action.lower())
        except Exception:
            return None
        if atype not in allowed_actions:
            return None
    
        return {'action': action, 'amount': amount, 'reasons': parsed.get('reasons', [])}
    ...
    def _make_betting_decision(
        ...
        # Build dynamic constraints based on actual valid actions
        constraint_lines = []
        for action in allowed_actions:
            if action == ActionType.BET:
                ...
                constraint_lines.append(f"- If action is \"raise\": amount must be 1-{max_raise}")
            elif action in [ActionType.CHECK, ActionType.CALL, ActionType.FOLD, ActionType.ALL_IN]:
                constraint_lines.append(f"- If action is \"{action.value}\": amount must be null")
    ...
    - Reasons should be concise, strategic justifications.
    {sb_hint}
    ...
    Start your response with the opening brace:"""
        if self.log_prompts:
            ...
            self.logger.info(decision_prompt)
        ...
        if bluffable and has_aggressive:
            ...
            if maybe:
                decision = maybe
                if self.verbose:
                    self.logger.info(f'{self.name}: Converted to bluff: {decision}')
...
llama_3_3_70B_client = OpenRouterCompletionEngine(model='meta-llama/llama-3.3-70b-instruct')


_log_prompts = False

player_open_ai_gpt_4o = Player(name='Open Ai Gpt-4o', chips=250, agent=LLMAgent(client = open_ai_gpt_4o_client, name='Open Ai Gpt-4o', log_prompts = _log_prompts))
player_anthropic_claud_sonnet_4_client = Player(name='Claude Sonnet 4', chips=250, agent=LLMAgent(client = anthropic_claud_sonnet_4_client, name='Claude Sonnet 4', log_prompts = _log_prompts))
player_google_gemini_flash_2_5 = Player(name='Gemini Flash 2.5', chips=250, agent=LLMAgent(client = google_gemini_flash_2_5_client, name='Gemini Flash 2.5', log_prompts = _log_prompts))
player_llama_3_3_70B_client = Player(name='Llama 3.3 70B', chips=250, agent=LLMAgent(client = llama_3_3_70B_client, name='Llama 3.3 70B', log_prompts = _log_prompts))
