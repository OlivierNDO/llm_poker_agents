...
class Hand:
    ...
    def _award_with_side_pots(self, active_players: List['Player']) -> str:
        ...
        # Evaluate hands for active players only (only they can win)
        player_hands = []
        for player in active_players:
            all_cards = [player.hole_cards[0], player.hole_cards[1]] + self.game_state.board
            rank, tiebreakers = HandEvaluator.evaluate_hand(all_cards)
        ...
        # FIXED: Get contribution levels from ALL players (including folded ones)
        # This ensures we account for all chips that went into the pot
        ...
        for level in contribution_levels:
            # FIXED: Calculate eligible players from ALL contributions, not just active players
            # Count how many players (active or folded) contributed at least this level
            total_players_at_level = sum(1 for contribution in all_contributions if contribution >= level)
            
            # But only active players can actually win the pot
            eligible_winners = [ph for ph in player_hands if ph['contribution'] >= level]
            
            pot_increment = level - last_level
            side_pot_size = pot_increment * total_players_at_level  # Use ALL players for pot size
            
            if not eligible_winners:
                # Edge case: all players at this level folded
                # This shouldn't happen in normal play, but handle gracefully
                self.logger.warning(f"No eligible winners for side pot level {level}")
                remaining_pot -= side_pot_size
                last_level = level
                continue
            
            # Find best hand among eligible winners
            eligible_sorted = sorted(eligible_winners, 
        ...
        if remaining_pot != 0:
            self.logger.error(f"ERROR: {remaining_pot} chips unaccounted for in pot distribution")
            self.logger.error(f"Original pot: {self.game_state.pot}")
            self.logger.error(f"Player contributions: {self.game_state.player_pot_contributions}")
            self.logger.error(f"Contribution levels: {contribution_levels}")
            
            # Emergency fix: award remaining chips to first winner
            if remaining_pot > 0 and contribution_levels:
                emergency_winner = player_hands[0]['player']  # Fallback to first player
                emergency_winner.chips += remaining_pot
                result_lines.append(f"  EMERGENCY: {emergency_winner.name} gets remaining {remaining_pot} chips")
...
class BetSizingAnalyzer:
    ...
    def __init__(self, player: Player, game_state: GameState, action_type: ActionType, table_config: TableConfig):
        self.player = player
        self.game_state = game_state
...
_log_prompts = False
_n_chips = 250
config = TableConfig(small_blind=1, big_blind=2, max_hands=500)
stats = StatsTracker()
